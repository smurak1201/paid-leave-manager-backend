...
class LeaveUsageController extends Controller
{
  ...
  public function index(Request $request)
  {
    ...
    try {
      if ($user->role === 'admin') {
        // 管理者は全件取得
        $usages = LeaveUsage::orderBy('employee_id')->orderBy('used_date')->get(['id', 'employee_id', 'used_date']);
  ...
  public function showSummary(Request $request)
  {
    ...
    // 閲覧ユーザーは自分の従業員データのみ
    if ($user->role === 'viewer') {
      if ($user->employee_id !== $validated['employee_id']) {
        return response()->json(['message' => '権限がありません'], 403);
      }
    }
    ...
    if (count($grant_details) > 1) {
      $carry_over = max(0, min($prev_remain, 20));
    } else {
      $carry_over = 0;
    }
    $used_dates_valid = [];
